...
"""
Financial Health & Credit Simulation Service

Consolidates two formerly separate modules:
  • financial_health.py  — 5-factor health score (spending, savings, credit,
                           loans, risk) written to FinancialHealthScore SQL model
  • simulation_service.py — stateless credit score impact simulation and credit
                            portfolio health analysis (no ORM writes)

Keeping them together makes sense because both answer the same question:
"How is the user's financial health, and how will a decision affect it?"
"""
...
# ── Credit Simulation (merged from simulation_service.py) ─────────────────────
...
def simulate_financial_impact(
    ...
    """
    Simulate the credit-score impact of a financial decision using deterministic
    rules.  Fully stateless — no ORM access, no LLM calls.

    Supported scenario_type values:
      new_loan / personal_loan / home_loan / auto_loan
      new_credit_card / credit_card
      missed_emi / default / missed_payment
      loan_payoff / early_closure / payoff

    Returns:
      impact_points, new_score, risk_level, score_trend, analysis,
      key_observations, recommendations, critical_warnings
    """
    ...
    if stype in {"new_loan", "personal_loan", "home_loan", "auto_loan"}:
        impact_points = -15
        risk_level = "MEDIUM" if amount > 500_000 else "LOW"
        score_trend = "DECLINING"
        observations.append(f"Applying for a ₹{amount:,.0f} loan triggers a hard inquiry.")
        observations.append("It temporarily lowers your score and increases credit utilization.")
        recommendations.append("Don't apply for multiple loans within a 6-month period.")
        recommendations.append("Set up auto-pay to ensure timely EMI payments once approved.")
        if amount > 1_000_000:
            warnings.append("High loan amount significantly impacts your debt-to-income ratio.")
    ...
    # 2. Credit Card Application
        ...
        risk_level = "LOW"
        score_trend = "STABLE"
        observations.append("New credit card applications cause a minor temporary score drop.")
        observations.append("Long-term it improves your credit mix and total available limit.")
    ...
    # 3. Missed Payment / Default
        ...
        impact_points = max(-50, -int(amount * 0.005) - 30)
        risk_level = "HIGH"
        score_trend = "DECLINING"
        observations.append(
            f"Missing a ₹{amount:,.0f} payment is a severe negative event on your credit report."
        )
        recommendations.append("Pay the due amount immediately including any late fees.")
        recommendations.append(
            "Contact your lender to negotiate a repayment plan if facing hardship."
        )
        warnings.append("Missed payments can stay on your credit report for up to 7 years.")

    # 4. Loan Payoff / Early Closure
    elif stype in {"loan_payoff", "early_closure", "payoff"}:
        impact_points = +15
        risk_level = "LOW"
        score_trend = "IMPROVING"
        observations.append(
            f"Paying off ₹{amount:,.0f} substantially reduces your outstanding debt burden."
        )
        recommendations.append("Obtain a No Objection Certificate (NOC) from the bank.")
        recommendations.append(
            "Keep your oldest credit accounts open to maintain credit history length."
        )

    # 5. Generic / Unknown Scenario
    else:
        impact_points = -5
        risk_level = "MEDIUM"
        score_trend = "STABLE"
        observations.append(f"Scenario '{scenario_type}' involving ₹{amount:,.0f}.")
        recommendations.append("Monitor your credit report regularly via CIBIL or Experian.")

    new_score = max(300, min(900, current_score + impact_points))
    direction = "reduce" if impact_points < 0 else "increase" if impact_points > 0 else "maintain"
    analysis = (
        f"This scenario is estimated to {direction} your score by {abs(impact_points)} points. "
        f"Overall risk is {risk_level} and trend is {score_trend.title()}."
    )
...
def analyze_credit_health(loans: list, current_score: int) -> dict:
    """
    Analyse a user's full credit profile and loan portfolio deterministically.

    Args:
        loans:         List of loan dicts, each with optional keys:
                         'principal', 'emi', 'missed_emis'
        current_score: User's current credit (CIBIL) score

    Returns:
        risk_level, score_trend, predicted_score_range, analysis,
        key_observations, recommended_actions, critical_warnings
    """
    total_principal = 0.0
    total_emi = 0.0
    missed_payments = 0
    ...
    for loan in (loans or []):
        try:
            total_principal += float(loan.get("principal") or 0)
            total_emi += float(loan.get("emi") or 0)
            missed = loan.get("missed_emis") or 0
    ...
    if missed_payments > 0:
        ...
        predicted_score_range = (
            f"{max(300, current_score - 40)} - {max(300, current_score - 10)}"
        )
    elif total_emi > 50_000:
        risk_level = "MEDIUM"
        score_trend = "STABLE"
        analysis = "High debt obligation, but no reported missed payments."
        predicted_score_range = (
            f"{max(300, current_score - 5)} - {min(900, current_score + 15)}"
        )
    elif not loans:
        risk_level = "LOW"
        score_trend = "STABLE"
        analysis = (
            "No active loans reported. Risk is low, though a healthy credit mix "
            "helps build your score over time."
        )
        ...
        predicted_score_range = (
            f"{min(900, current_score + 10)} - {min(900, current_score + 25)}"
        )
    ...
    if loans:
        observations.append(
            f"You hold {len(loans)} active loan(s) with a total EMI of ₹{total_emi:,.0f}."
        )
    else:
    ...
    if missed_payments > 0:
        warnings.append(
            f"Critical: {missed_payments} missed payment(s) detected. "
            "Pay immediately to stop compounding penalties."
        )
        actions.append("Clear overdue EMIs within 30 days to mitigate CIBIL damage.")
    else:
        actions.append("Continue paying all EMIs on time, before the due date.")

    if current_score < 600:
        warnings.append(
            "Score is well below average — new credit will be difficult and expensive to obtain."
        )
        actions.append("Focus entirely on repayment; do not apply for any new credit.")
    elif current_score > 750:
        observations.append(
            "Your score is excellent and qualifies you for the best market interest rates."
        )
